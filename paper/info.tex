...
(additive) identity; see, for example, \cite{Lothaire2005, Sakarovitch2009}. We
...
\begin{theorem}
  ...
  semigroup, and let \(\sigma, \tau\in\Sym(S)\). Then $\phi\in \Aut(S, +)$ is a
  semigroup isomorphism from \((S,\times^\sigma)\) to \((S, \times ^ \tau)\) if
...
$\phi: S \to S$ such that $(x\times y)\phi = y\otimes x$ for all
$x, y\in S$.
...
\begin{theorem}
  ...
  Then \(\phi\in \Aut(S, +)\) is a semigroup isomorphism or
  anti-isomorphism from \((S, \times ^ \sigma)\) to \((S, \times^\tau)\)
...
computers. The longest running of the computation in the tables below
